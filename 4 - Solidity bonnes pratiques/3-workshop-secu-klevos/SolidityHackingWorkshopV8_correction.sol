...
contract Store {
    ...
    /**
        Correction
        Toute la fonction surconsomme ! c'est d'ailleurs le pb
        DoS gas
...
contract LinearBondedCurve {
    ...
    function sell(uint _amount) public {
        totalSupply -= _amount;                                                 // changer le totalsupply  avant le calcul
...
contract Coffers {
    ...
    function closeAccount() external {
        ...
        for (uint i=0; i<coffer.nbSlots; ++i){
            ...
            //mapping slot n'est pas remis a 0
            delete coffer.slots[i];
...
contract CommonCoffers {
    ...
    function deposit(address _owner) payable external {
        require(msg.value > 0);
        if (scalingFactor != 0) {
            uint toAdd = (scalingFactor * msg.value) / (address(this).balance - msg.value);     //si balance = msg.value revert peut avoir nb negatif
    ...
    function withdraw(uint _amount) external {
        uint toRemove = (scalingFactor * _amount) / address(this).balance;          // renvoie 0 si plus de 100
